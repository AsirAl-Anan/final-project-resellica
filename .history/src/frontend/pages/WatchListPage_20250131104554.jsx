...
const Watchlist = () => {
  ...
  const handleRemoveFromWatchlist = async (productId) => {
    try {
      ...
      fetchWatchlistItems(); // Refresh the list after removing
    } catch (error) {
      console.error('Error removing from watchlist:', error);
    }
  };

  // Add this to product cards in both Watchlist and Product list pages
  const WatchlistButton = ({ product }) => {
    const [isInWatchlist, setIsInWatchlist] = useState(product.isInWatchlist);
    
    const handleClick = async () => {
      try {
        const action = await handleWatchlist(product);
