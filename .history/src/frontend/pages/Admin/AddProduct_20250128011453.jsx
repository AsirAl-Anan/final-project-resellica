...
const ProductForm = () => {
  ...
  // ... keep existing functions (fetchCategories, handleCategoryChange, etc.) ...
          ...
          {productData.originalPrice && productData.sellingPrice && (
            <div className="mt-2">
              <span className="bg-purple-100 text-purple-800 text-xs font-medium px-2 py-1 rounded">
                Save: ₮{productData.originalPrice - productData.sellingPrice}
              </span>
            </div>
          )}
            ...
            {steps.map((step, index) => (
              <div
                ...
                className={`flex items-center p-4 cursor-pointer rounded-lg mb-2 ${
                  activeStep === index ? 'bg-purple-50 text-purple-600' : 'text-gray-500'
                }`}
                onClick={() => setActiveStep(index)}
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                  activeStep === index ? 'bg-purple-600 text-white' : 'bg-gray-100'
                }`}>
                  <step.icon size={20} />
                </div>
                <div className="ml-3">
