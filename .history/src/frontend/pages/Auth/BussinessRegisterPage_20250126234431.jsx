...
export default function BusinessRegisterPage() {
  const { user, error, message, emailVerificationSent, emailVerifying, dbUser, signupBussinessAccount, reloadUserData, getUserData } = useContext(AuthContext);
  ...
  useEffect(() => {
    ...
    if (dbUser && dbUser.length > 0 && dbUser[0]?.role === "seller") {
      console.log('inside role checker useEffect');
  ...
  }, [user?.emailVerified, isNewSignup, message, navigate, user, dbUser]);
 //for reloading
  useEffect(() => {
    getUserData();
    if (dbUser && dbUser.length > 0 && dbUser[0]?.role === "seller") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await signupBussinessAccount(user, country);
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
          <div className="max-w-md w-full space-y-8">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              >
                {loading ? "Hold on..." : "Register your account to sell items"}
              </button>
            </form>
          </div>
        </div>
