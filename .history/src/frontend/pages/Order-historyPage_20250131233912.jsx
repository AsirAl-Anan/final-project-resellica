  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const addressResponse = await api.get(`/users/api/v1/get-address/${userResponse.data.data._id}`);
          if (addressResponse.data && addressResponse.data.data) {
            setAddress(addressResponse.data.data.addressLine || '');
            setAddressExists(true);
          }
        } catch (addressError) {
          if (addressError.response && addressError.response.status === 404) {
            // Address not found, which is fine
        ...
        <div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              ...
              />
              {!addressExists && (
                <p className="text-sm text-gray-500 mt-1">No address found. Please enter your address.</p>
              )}
            </div>
            <button
              type="submit"
