...
const Watchlist = () => {
  ...
  const fetchWatchlistItems = async () => {
    try {
      ...
      // Updated to handle the nested data structure
      const data = response.data.data.watchlist;
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {watchlistItems.map((item) => (
              <div key={item.product._id} className="bg-white rounded-lg shadow overflow-hidden">
                <div className="relative h-48">
                  ...
                  <div className="absolute top-2 right-2">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                      {item.product.watchlistCount || 0} watching
                    </span>
                  </div>
                </div>

                <div className="p-4">
                  <h4 className="text-lg font-medium text-gray-900 mb-2">{item.product.title}</h4>
                  
                  <div className="flex items-baseline space-x-2">
                    <span className="text-2xl font-bold text-purple-600">
                      {item.product.currency} {item.product.resalePrice?.toLocaleString()}
                    </span>
                  ...
                  {item.product.originalPrice && item.product.resalePrice && (
                    <div className="mt-2">
                      <span className="bg-green-100 text-green-800 text-xs font-medium px-2 py-1 rounded">
                        Save {item.product.currency} {(item.product.originalPrice - item.product.resalePrice).toLocaleString()}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="mt-4 space-y-2 text-sm text-gray-600">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-sky-500">
                        {item.product.seller?.username || 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Location:</span>
                      <span className="font-medium">
                        {item.sellerAccount?.[0]?.country || 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Years of use:</span>
                      <span className="font-medium">{item.product.yearsOfUse || 'N/A'}</span>
                    </div>
                  </div>

                  <div className="text-sm text-gray-500 mt-2">
                    Added {formatTimeAgo(item.product.createdAt)}
                  </div>

                  <div className="mt-4 space-y-2">
                    <button
                      onClick={() => handleRemoveFromWatchlist(item.product._id)}
