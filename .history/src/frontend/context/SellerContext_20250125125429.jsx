...
const SellerAuthProvider = ({ children }) => {
    ...
    const checkEmailVerification = async () => {
      try {
        ...
        if (currentUser.emailVerified) {
          ...
          setEmailVerifying(false);
          setMessage('Email verified and bussiness account created successfully');
          ...
          // Continue polling if email isn't verified   setLoading(
  ...
  const signup = async (email, password, username ,fullname) => {
    ...
    const checkEmailVerification = async () => {
      try {
        ...
        if (error.code === 'auth/user-token-expired') {
          // Stop polling if token expired    setLoading(
          setEmailVerifying(false);
          setError('Email verification session expired. Please sign in again.');
          return;
        }
  ...
  const signin = async (email, password,) => {
    try {
      ...
      if(response.status === 404){    setLoading(
        setError("User not found")
      }
      if(response.status === 403){    setLoading(
        setError("Incorrect password")
      }
      if(response.status === 200){    setLoading(
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (seller) {
        try {
          ...
          if (response.status === 200) {    setLoading(
            setDbSeller(response.data.data[0]); // Update dbUser
          }
    ...
    fetchUserData();
  }, [seller]); // Only run when `user` changes
