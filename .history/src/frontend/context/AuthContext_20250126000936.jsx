...
const AuthProvider = ({ children }) => {
  ...
  const getUserData =async () =>{
    ...
    return  setDbUser(response.data.data);
    }
  }
        ...
        try {
          ...
          if (currentUser.emailVerified) {
            ...
            if (registerResponse.status === 200) {
              ...
              const userDataResponse = await api.get("/users/api/v1/get-user-data");
   console.log("hscu",userDataResponse)
              if (userDataResponse.status === 200) {
                // Set the dbUser state with the fetched data
                setDbUser(userDataResponse.data.data);
              }
  
              // Step 7: Update the user's profile with the avatar if available
              if (!currentUser.photoURL && userDataResponse.data.data.avatar) {
                await updateProfile(currentUser, {
                  photoURL: userDataResponse.data.data.avatar
                });
              }
            }
