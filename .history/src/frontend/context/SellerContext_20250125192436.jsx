...
const SellerAuthProvider = ({ children }) => {
  ...
  const signin = async (email, password, verificationCode) => {
    try {
      ...
      const currentSeller = auth.currentUser;
      if (!currentSeller) {
        setError("Unable to retrieve user details. Please try again.");
        return null;
      }
  
      // Reload the user to ensure the latest email verification status
      await reload(currentSeller);
      ...
      // Check the verification code and database registration
      const response = await api.post(
        "/sellers/api/v1/login",
        {
          email,
          password,
          verificationCode,
      ...
      if (response.status === 200) {
        ...
        setEmailVerificationSent(false);
        setEmailVerifying(false);
        setError("Login failed. Please check your credentials or verification code.");
        return null;
      }
