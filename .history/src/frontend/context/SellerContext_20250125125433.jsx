...
const SellerAuthProvider = ({ children }) => {
    ...
    const checkEmailVerification = async () => {
      try {
        ...
        if (currentUser.emailVerified) {
          ...
          // Continue polling if email isn't verified   setLoading(false)
  ...
  const signup = async (email, password, username ,fullname) => {
    ...
    const checkEmailVerification = async () => {
      try {
        ...
        if (error.code === 'auth/user-token-expired') {
          // Stop polling if token expired    setLoading(false)
          setEmailVerifying(false);
          setError('Email verification session expired. Please sign in again.');
          return;
        }
  ...
  const signin = async (email, password,) => {
    try {
      ...
      if(response.status === 404){    setLoading(false)
        setError("User not found")
      }
      if(response.status === 403){    setLoading(false)
