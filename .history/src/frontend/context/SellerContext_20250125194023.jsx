...
const SellerAuthProvider = ({ children }) => {
  ...
  const signin = async (email, password) => {
    try {
      ...
      const currentSeller = sellerCredential.user;
      const response = await api.get('/sellers/api/v1/get-seller', {email})
      if(!currentSeller || response.status === 404){
        setError('Seller not foun')
      }
