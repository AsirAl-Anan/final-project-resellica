...
const SellerAuthProvider = ({ children }) => {
  ...
  const signin = async (email, password) => {
    try {
      ...
      if(sellerResponse.status === 200){
        sellerCredential = await signInWithEmailAndPassword(auth, email, password);
        currentSeller = sellerCredential.user;
      }
  ...
  const signout = async () => {
    try {
      setError(null);
      await api.post('/seller/api/v1/logout')
      await signOut(auth);
    } catch (err) {
