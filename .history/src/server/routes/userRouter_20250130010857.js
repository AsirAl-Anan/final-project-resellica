 ...
 userRouter
  ...
  userRouter.post('/api/v1/get-products/:category', async (req, res, next) => {
    try {
        ...
        // Debug: Check if there are any products before pipeline
        const totalProducts = await productModel.countDocuments(matchStages);
        console.log("Total products before pipeline:", totalProducts);

        // Execute the aggregation pipeline
        const products = await productModel.aggregate(pipeline);
        console.log("Products after pipeline:", products.length);
        
        // Debug: Check pipeline stages results
        for (let i = 0; i < pipeline.length; i++) {
            const intermediateResults = await productModel.aggregate(pipeline.slice(0, i + 1));
            console.log(`After pipeline stage ${i}:`, {
                stage: pipeline[i],
                resultCount: intermediateResults.length
            });
        }
        ...
        const [totalCount] = await productModel.aggregate(pipeline);
