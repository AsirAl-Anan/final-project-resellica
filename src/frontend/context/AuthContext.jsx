  ...
  const fetchAdminData = async () => { 
    ...
    const signupWithGoogle = async () => {
      try {
        ...
        // Step 1: Initialize GoogleAuthProvider
        const provider = new GoogleAuthProvider();
    
        // Step 2: Trigger Google Sign-In Popup
        ...
        // Step 4: Check if the user already exists in MongoDB
        console.log("Checking if user exists in MongoDB...");
        const checkUserResponse = await api.get(`/users/api/v1/check-user?email=${email}`);
        
        if (checkUserResponse.data.exists) {
          // If the user already exists, throw an error and do not proceed
          toast.error("User already exists in the database.");
          setTimeout(1000,()=>{
           return window.location.href = "/user-register"
            
          })
          return null 
        }
    
        // Step 5: Register user in backend (MongoDB)
        ...
        if (registerResponse.status === 201) {
          ...
          // Step 7: Set verification states
          setEmailVerificationSent(false);
          setEmailVerifying(false);
          ...
          console.log("Deleting Firebase user due to failed registration...");
          try {
            await deleteUser(googleUser); // Ensure deleteUser is an async function
            console.log("Firebase user deleted successfully.");
          } catch (deleteError) {
            console.error("Failed to delete Firebase user:", deleteError);
            setError('Failed to delete Firebase user after registration failure');
          }
        }
        ...
        switch (error.code) {
          ...
          case 'auth/cancelled-popup-request':
            setError('Sign-up request was cancelled');
            break;
          default:
